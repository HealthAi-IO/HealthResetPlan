...
Future<bool> ensureAiConsent(BuildContext context) async {
  ...
  final accepted = await showDialog<bool>(
    ...
    builder: (context) => AlertDialog(
      ...
      content: const Text(
          '使用健康管家个性化模式时，系统会按需读取你的健康档案、近期指标、近 7 天饮食、今日计划以及你主动保存的管家记忆，并连同本次问题由受控服务器转发给 AI 服务商生成回答。你可以切换为不读取个人数据的通用模式，也可以随时查看、修改或删除管家记忆。图片仅在你主动选择后用于对应分析，不用于训练；请求和回答不写入运营数据、审计日志或明文数据库，管理后台无法查看。AI 仅供健康管理参考，不能替代医生诊断。'),
      actions: [
        TextButton(
