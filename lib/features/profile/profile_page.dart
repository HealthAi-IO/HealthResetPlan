    ...
    final action = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(accepted
            ...
            : '健康管家个性化模式会按需读取你的健康档案、近期指标、近 7 天饮食、今日计划和你主动保存的管家记忆，并由受控服务器转发给 AI 服务商处理。你可以切换为通用模式并随时管理记忆。图片仅在主动选择后用于对应分析并加密保存；请求正文、AI 回答和图片不写入运营数据、审计日志或明文数据库，管理后台无法查看。AI 仅提供健康管理参考，不能替代医生诊断。'),
        actions: [
          TextButton(
                ...
                child: Column(
                  children: [
                    ...
                    ListTile(
                      ...
                      title: const Text('AI 健康权益'),
                      subtitle: const Text('了解权益并购买 AI 分析次数'),
                      trailing: const Icon(Icons.chevron_right),
                      onTap: () => context.push('/ai-benefits'),
                    ),
                  ],
                ),
              ),
